...
static ssize_t pcm_read(struct recorder *rec, size_t rcount)
{
	...
	data = rec->audiobuf;  // 读取的音频数据存放在audiobuf中
	while (count > 0) {
		r = snd_pcm_readi(handle, data, count);    // 读取的音频数据存放在audiobuf中
...
static void * record_thread_proc(void * para)
{
	...
	while(1) {
		frames = rec->period_frames;   // frames = 1600
		bytes = frames * rec->bits_per_frame / 8;   // 每一次读取音频数据字节数 
		...
		if (pcm_read(rec, frames) != frames) {      // 读取的音频数据存放在 rec->audiobuf 中
			return NULL;
		}
		if (rec->on_data_ind)
